...
Until Sep 7, DSA owns weekday evenings and weekend mornings. HLD lives in slots DSA can't use:
...
**From Sep 7, the budget flips: HLD inherits weekday evenings** (DSA's 11 weeks end), and DSA drops to a maintenance dose (see Phase 3).
...
- **~Aug 10–17:** recruiter outreach / applications. **Start warming referrers NOW — referrals beat cold applies and cost zero study time.**
- **Sep 8–19: phone screens.** REQUEST this window explicitly; recruiters usually accommodate. Screens are *mostly* coding — but confirm per company: Amazon screens are LP-heavy, and some Lead-level pipelines include a design or hiring-manager screen. **Contingency: a screen forced earlier than Sep 8 outranks that week's Phase-2 reading. Keep one screen-grade Tier-C design warm from mid-Aug (News Feed, post Aug 9).**
- **Sep 28 – Oct 16: onsites** — HLD exit criteria green by Sep 27, DSA kept warm by the maintenance dose below.
- Decide-by **2026-07-12** (with PLAN.md's timeline call): target company list. **The list gates LLD:** if Amazon / Atlassian / Uber-tier machine-coding rounds are on it, an LLD mini-track (4–6 evening sessions) is added to Phase 3 and the onsite window shifts ~1 week right. If not, the de-scope gets written down here, with its consequence (don't interview at machine-coding shops).
...
- [ ] 4 human design mocks (calibration Jul 18/19 · Sep 12/13 · Sep 19/20 · final Sep 25/26); **last 2 verdict "would advance at Lead/Senior level."**
...
## CURRENT POSITION
- **Phase:** Pre-start. NOTHING begins before Mon 2026-07-06 — the DSA consolidation sprint owns Jul 2–5 completely.
- **First actions:** Tue Jul 7 lunch → `concept H1` (+ Hello Interview framework read). Thu Jul 9 lunch → `concept H6a+H3a`. Sun Jul 12 afternoon → `design rate-limiter` (rep #1). **Book by Jul 12:** the calibration design mock (interviewing.io / Hello Interview) for Jul 18/19.
- **Decide by Jul 12:** target company list → gates the LLD mini-track (see Pipeline). Mock budget rule: ≤2 human mocks/week across BOTH tracks.
